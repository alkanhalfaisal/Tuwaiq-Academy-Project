...
main() {
  ...
  print(" ----------------------------------");
  print("|  1- Add book                     |");
  print("|  2- Show all books in BookStore  |");
  print("|  0- Exit                         |");
  print(" ----------------------------------");
    ...
    case '1':
      {
        ...
        PhysicalBook addedBook = PhysicalBook(ourStore.books.length.toString(),
            title, Quantity_int, price_double, author);
...
class BookStore {
  ...
  addBook(Book addBook) {
    books.add(addBook);
    print('${addBook} added successfully');
  }
  ...
  removeBookById(String id) {
    if (searchByTitle(id).runtimeType.toString() == 'Book') {
      books.remove(searchById(id));
    } else {
      print('Sorry we don\'t have any book with id: ${id} currently');
    }
  }
  ...
  searchById(String id) {
    for (Book book in books) {
      if (book.id == id) {
        ...
        print('Sorry we don\'t have book with this ${id} currently');
      }
    }
  }
  ...
  listAllBook() {
    for (Book book in books) {
      print(
          "ID: ${book.id}     Book Name: ${book.title}               Author Name: ${book.author}               type: ${book.runtimeType.toString()}");
    }
  }
