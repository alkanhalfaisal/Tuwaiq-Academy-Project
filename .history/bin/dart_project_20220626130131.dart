...
main() {
  int uniqueID = 6; // it unique id for each book
  BookStore ourStore =
  ...
  do {
    ...
    var Userchoice = (stdin.readLineSync().toString());
    switch (Userchoice) {
      case '1': //Add PhysicalBook
        {
          print('>: Please enter title of the book you want to add');
          String title = (stdin.readLineSync().toString());
          print(
              '>: Please enter Quantity of the book you want to add *Please enter integer number');
          String Quantity = (stdin.readLineSync().toString());
          int Quantity_int = int.parse(Quantity);
          print(
              '>: Please enter price of the book you want to add *Please enter number');
          String price = (stdin.readLineSync().toString());
          double price_double = double.parse(price);
          print('>: Please enter author name of the book you want to add');
          String author = (stdin.readLineSync().toString());
          PhysicalBook addedBook = PhysicalBook(
              (uniqueID++)
                  .toString(), // we assign unique ID automatically and increase one each time we add item
              title,
              Quantity_int,
              price_double,
              author);
      ...
      case '5': //search a book in BookStore
        {
          ...
          if (User == 'Title' || User == 'title') {
            ...
            User = (stdin.readLineSync().toString());
            if (ourStore.searchByTitle(User) != -1) {
              // check if the bokk is exits
    ...
    print(Userchoice);
  } while (Userchoice != '0'); // if the user enter 0 we will close the app
}
